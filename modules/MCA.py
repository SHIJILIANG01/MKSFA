...
class Moment_efficient(nn.Module):  #
    def forward(self, x):
        avg_x = torch.mean(x, (2, 3)).unsqueeze(-1).permute(0, 2, 1)  # [1,1,256]
        std_x = torch.std(x, (2, 3), unbiased=False).unsqueeze(-1).permute(0, 2, 1)  # [1,1,256]
        moment_x = torch.cat((avg_x, std_x), dim=1)  # bs,*,c [1,2,256]
...
class MomentAttention_v1(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.moment = Moment_efficient()  # [1,2,256]
        self.c = ChannelAttention()  # [1,1,256]

    def forward(self, x):
        y = self.moment(x)  # bs,2,c
        result = self.c(y)  # bs,1,c
        result = result.permute(0, 2, 1).unsqueeze(-1)  # bs,c,1,1
        output = x * result.expand_as(x)   # [1,256,32,32]
